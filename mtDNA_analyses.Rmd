...
for(m in list.files(path=wd,pattern="AllIndividuals_rep[0-9]+.nex"))
  ...
  pwtest<-pairwiseTest(gt,nrep = 0, stats="fst",quietly=T) # calculate pairwise Phist. change back to phist
  phist_mtdna<-as.dist(pwtest$pair.mat$Fst) #grab the Phist table #change back to phist
  phist_mtdna[which(is.na(phist_mtdna))]<-0 #set any NAs (due to identical hap freqs) in the table to zero
  ibd<-mantel.randtest(phist_mtdna,latdists) # calculate Mantel test
  ibds[[m]]<-ibd
  }
...
ggsave("mantelhist_theta_stepstone_g10k_Nl4_m0.1.pdf",mantelhist)
