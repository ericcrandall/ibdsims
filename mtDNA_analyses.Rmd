...
c <- ape::read.nexus.data("./pareto2_mtdnas/pareto2_AllIndividuals_rep1_loc2.nex")
d <- as.DNAbin(c)
e <- dist.dna(d)
h <- pegas::haplotype(d)
h <- sort(h, what = "label")
...
legend(-1000, -40, colnames(ind.hap), col=rainbow(ncol(ind.hap)), pch=19, ncol=2)
```

#maybe convert to gtypes with pop as populations, then write to arlequin format, then convert to migrate format...
